...
# Strategy Compare履歴一覧でのDashboard比較用チェック選択状態（Phase14C）。
# 値は選択中のcompare_run_idの集合（set[str]）。Runner履歴用の
# _SS_KEY_WF_DASHBOARD_SELECTION（Phase14B）とは独立したキーとし、
# 互いに上書きしない。新しいキャッシュ機構ではなく、既存の
# st.session_stateを1スロット分利用するだけ（Phase14Bと同じ設計）。
_SS_KEY_WF_COMPARE_DASHBOARD_SELECTION = "walkforward_compare_dashboard_selected_compare_run_ids"
# 「📊 選択した比較履歴をDashboardで比較」ボタン押下時に組み立てた
# named_resultsの保持スロット（Phase14C）。ボタンを再度押すまでは
# 再描画のたびにload_compare_result()を再実行しない
# （_SS_KEY_WF_DASHBOARD_RESULTと同じ設計思想）。
_SS_KEY_WF_COMPARE_DASHBOARD_RESULT = "walkforward_compare_dashboard_named_results"
...
# ════════════════════════════════════════════════
# Runner結果のSQLite保存ボタン（Blocking-1: 欠落していた実装を復元）
# ════════════════════════════════════════════════
def render_walkforward_save_button(runner_result: dict) -> None:
    """
    現在表示中のRunnerResult（単体）をSQLiteへ保存するボタンを表示する。

    walkforward_storage.save_runner_result()（無変更）を呼ぶだけの
    独立した保存経路であり、Strategy Compare結果全体の保存
    （render_walkforward_compare_save_button）とは責務を混在させない
    （両者はテーブルもキーも独立しており衝突しない設計。同関数と
    対称の構成とする）。

    render_walkforward_runner_result() から show_save_button=True の
    場合のみ呼ばれる。show_save_button=False（Strategy Compare配下の
    戦略別詳細表示）から呼ばれることはない（Critical-2対応。
    render_walkforward_runner_result()のdocstring参照）。

    Args:
        runner_result: run_walkforward_runner() の戻り値。
    """
    if st.button("💾 SQLiteへ保存", key="wf_save_to_sqlite_button"):
        try:
            saved_id = save_runner_result(runner_result)
            st.success(f"✅ 保存しました（run_id: {saved_id}）")
        except sqlite3.IntegrityError:
            st.warning("⚠️ このRunner結果（run_id）は既に保存済みです。上書きは行われません。")
...
def render_walkforward_strategy_compare_section(
    ...
    # Compare結果全体の保存ボタン（Compare結果保存機能の状態確認と
    # 必要最小限の修正）。render_walkforward_compare_result_body()は
    # ライブ実行・Compare履歴表示の両方から共有される表示関数のため、
    # その内部には置かない（同関数末尾のコメント参照）。履歴側
    # （render_walkforward_compare_history_section）は既に保存済みの
    # レコードを表示するだけなので、保存ボタンはここ、ライブ実行側にのみ
    # 表示する。
...
def render_walkforward_compare_history_section() -> None:
    ...
    # Compare履歴一覧でのDashboard比較用チェック選択状態（Phase14C。
    # Runner履歴側render_walkforward_history_tableのselected_run_idsと
    # 同じ設計思想。新しいキャッシュ機構ではなく、既存のst.session_state
    # を1スロット分利用するだけ）。
    selected_compare_run_ids: set = st.session_state.setdefault(
        _SS_KEY_WF_COMPARE_DASHBOARD_SELECTION, set()
    )
    ...
    render_history_table_generic(
        ...
        on_delete=_on_delete,
        checkbox_key_prefix="wf_compare_history_dashboard_check",
        selected_ids=selected_compare_run_ids,
    )

    st.divider()
    render_walkforward_compare_history_dashboard_selector(history_rows)
...
# ════════════════════════════════════════════════
# Compare履歴 → Dashboard比較（Phase14C。Phase14Bの横展開。Runner履歴は対象外）
# ════════════════════════════════════════════════
def render_walkforward_compare_history_dashboard_selector(history_rows: list[dict]) -> None:
    """
    Strategy Compare履歴一覧でチェック選択された行を、Walk Forward
    Analysis Dashboard（render_walkforward_analysis_dashboard、Phase14A・
    無変更）で横断比較するための導線。

    render_walkforward_history_dashboard_selector()（Phase14B・無変更）と
    可能な限り同一の設計とする。異なるのはただ1点で、Runner履歴の1行が
    RunnerResult1件に対応するのに対し、Compare履歴の1行
    （StrategyCompareResult）は compare_result["strategies"] 配下に
    複数戦略分のRunnerResultを含む点だけである。そのため本関数では、
    選択された各compare_run_idについて load_compare_result()
    （無変更）を1回呼び、その"strategies"を展開してnamed_resultsへ
    合流させる。StrategyCompareResultの解析・ランキング計算・
    status再計算等の新しいロジックは一切追加しない
    （render_walkforward_compare_result_body()が既に行っている
    strategies_result = compare_result.get("strategies") or {} と
    同じ読み取り方をそのまま踏襲するだけ）。

    「📊 選択した比較履歴をDashboardで比較」ボタンが押された時にのみ
    load_compare_result()を呼び出す。チェックボックスの選択操作自体では
    ロードを行わない（render_walkforward_compare_history_section側の
    checkbox処理を参照。Phase14Bと同じ設計）。

    表示名には、銘柄・戦略・保存日時に加えてcompare_run_idの先頭8文字を
    含める。Compare履歴では1回の選択で複数戦略（例: v8とv9）が同時に
    named_resultsへ展開されるため、Phase14Bの「銘柄+戦略+保存日時」
    だけでは、複数のCompare実行が同じ秒に保存された場合に表示名が
    衝突しうる（dictのキー衝突により片方が上書きされ、Dashboardの
    比較表から消えてしまう）。compare_run_idを含めることで、この
    衝突を確実に避ける。

    選択件数の上限（_WF_DASHBOARD_MAX_SELECTION、Phase14Bと共有）は
    「選択したCompare履歴の行数」に対して適用する（Phase14Bの
    「選択したRunner履歴の行数」と同じ数え方）。1件のCompare履歴は
    通常2〜3戦略程度を含むため、Dashboardへ最終的に渡される項目数は
    行数の上限より多くなりうる（例: 10行 × 2戦略 = 20項目）。これは
    新しい判定ロジックではなく、Runner履歴（1行=1項目）とCompare履歴
    （1行=N項目）という既存のデータ構造の違いによるものであり、
    Dashboard本体（render_walkforward_analysis_dashboard、無変更）は
    項目数に関わらず同じ描画処理を行う。

    Runner履歴（render_walkforward_history_section・
    render_walkforward_history_dashboard_selector）は、このセクション
    からも render_history_table_generic() のcheckbox機能からも
    一切変更していない。

    Args:
        history_rows: 現在表示中のStrategy Compare履歴一覧
            （list_compare_results() または search_compare_results() の
            戻り値）。選択中compare_run_idに対応する行のメタ情報
            （銘柄・期間・保存日時）を表示名の組み立てに使う。
    """
    selected_compare_run_ids: set = st.session_state.get(_SS_KEY_WF_COMPARE_DASHBOARD_SELECTION, set())
    # 現在の一覧に含まれない（検索条件変更等で見えなくなった）compare_run_idは
    # 比較対象から自然に除外する（Phase14Bと同じ方針）。
    visible_ids = {row.get("compare_run_id") for row in history_rows}
    active_selection = selected_compare_run_ids & visible_ids
    ...
    if not active_selection:
        st.info("履歴一覧の「比較」列にチェックを入れると、複数のCompare実行結果をDashboardで横断比較できます。")
        return

    if st.button("📊 選択した比較履歴をDashboardで比較", key="wf_compare_history_dashboard_compare_button"):
        rows_by_id = {row.get("compare_run_id"): row for row in history_rows}
        named_results: dict[str, dict] = {}
        for compare_run_id in active_selection:
            try:
                loaded = load_compare_result(compare_run_id)
            except Exception:
                st.error(f"❌ compare_run_id={compare_run_id} の読込中にエラーが発生しました。")
                st.code(traceback.format_exc(), language="text")
                continue
            if loaded is None:
                continue

            meta_row = rows_by_id.get(compare_run_id, {})
            strategies_result = loaded.get("strategies") or {}
            for strategy_name, runner_result in strategies_result.items():
                strategy_label = _dashboard_strategy_label(strategy_name)
                display_name = (
                    f"{meta_row.get('code') or '?'} {strategy_label} "
                    f"({meta_row.get('created_at') or '?'} / {compare_run_id[:8]})"
                )
                named_results[display_name] = runner_result

        st.session_state[_SS_KEY_WF_COMPARE_DASHBOARD_RESULT] = named_results

    dashboard_named_results = st.session_state.get(_SS_KEY_WF_COMPARE_DASHBOARD_RESULT)
    if dashboard_named_results:
        st.divider()
        render_walkforward_analysis_dashboard(dashboard_named_results)

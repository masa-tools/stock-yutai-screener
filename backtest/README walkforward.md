...
これに加えて、Pipelineが内部で保持している**`pipeline_result["warnings"]`（内部Stageのwarnings）も、Runnerの`warnings`へそのまま転記されます**（`{"stage": "pipeline", "message": w.get("message", str(w))}`という形）。つまり、Runnerの戻り値`warnings`には以下2種類の情報が混在します。

- **Pipeline内部の`warnings`**（そのまま転記。例: Validationウィンドウが1つも生成されなかった場合の警告等）
- **Pipeline内部の`errors`の要約**（`"内部Stage(...)でエラーが記録されています: ..."`という文言に整形して転記）

いずれも`{"stage": "pipeline", "message": ...}`という同じ形のエントリとして`warnings`配列に並ぶため、`message`の内容で両者を判別する必要があります。
...
Walk Forward一式には「`context`」という語が**2つの異なる意味**（Stage4の実行結果／将来拡張用の予約入力）で登場し、後者（予約入力）は転送先に応じて**複数箇所に同じ値が複製されます**（詳細は(C)参照）。将来のAPI設計・SQLite保存スキーマ設計において混同すると事故の原因になるため、必ず区別してください。
...
### (C) `context`という語が登場する4箇所（重要）

Runnerの`context`引数は、Benchmark・Summary・Contextの各Stageへ`context=context`としてそのまま転送されます（(B)参照）。転送された先のBenchmark・Summary自身の戻り値にも、**その予約入力をそのまま保持する`"context"`キー**が存在します。そのため、Walk Forward一式全体で見ると「`context`」という語は次の**4箇所**に登場します。

```mermaid
flowchart LR
    IN["Runner引数 context<br/>（予約入力そのもの）"] -->|そのまま転送| BM["② result['benchmark']['context']<br/>＝予約入力そのもの"]
    IN -->|そのまま転送| SM["③ result['summary']['context']<br/>＝予約入力そのもの"]
    IN -->|そのまま転送| CTX_CALL["build_walkforward_context() 呼び出し"]
    IN -->|指定時のみ格納| RUNNER_INPUT["④ result['context_input']<br/>＝予約入力そのもの"]
    CTX_CALL --> RESULT_CTX["① result['context']<br/>＝Stage4の実行結果（別物）"]
```

| # | キー | 中身 | Stage4結果か予約入力そのままか |
|---|---|---|---|
| ① | `result["context"]` | Stage4（`build_walkforward_context()`）の実行結果 | **Stage4結果**（`context_schema_version`等を持つ別構造） |
| ② | `result["benchmark"]["context"]` | Runner引数`context`の値 | **予約入力そのまま**（④と同じ値） |
| ③ | `result["summary"]["context"]` | Runner引数`context`の値 | **予約入力そのまま**（④と同じ値） |
| ④ | `result["context_input"]` | Runner引数`context`の値 | **予約入力そのまま**（②・③と同じ値） |

**②・③・④は同じ値（Runnerに渡した`context`引数そのもの）であり、①（Stage4の実行結果）とは完全に別物です。** `result["benchmark"]["context"]`や`result["summary"]["context"]`を見て「Stage4の結果の一部だろう」と誤解しないよう注意してください。②③④はいずれも「呼び出し側が渡した予約入力をそのまま複製しているだけ」であり、Walk Forward一式のどのモジュールもその中身を解釈していません（⑨将来拡張ポイント参照）。
...
| `result["benchmark"]["context"]` | Runner呼び出し時に渡した`context`引数の値（そのまま。`context_input`と同じ値） | `context`引数が指定され、かつBenchmark Stageが実行された場合のみ存在 |
| `result["summary"]["context"]` | Runner呼び出し時に渡した`context`引数の値（そのまま。`context_input`と同じ値） | `context`引数が指定され、かつSummary Stageが実行された場合のみ存在 |
...
## ⑬ 戦略評価機能（Strategy Compare / Window可視化 / CSV / ランキング）

「開発者向け検証ツール」から「戦略評価ツール」への発展フェーズ（Phase1〜2前半）として、以下4機能を追加しました。既存8モジュール（`walkforward.py`〜`walkforward_context.py`）・`walkforward_runner.py`・`types.py`は無変更です。

### Strategy Compare（`backtest/walkforward_strategy_compare.py`）
`run_walkforward_runner()`を戦略の数だけ呼び出し、結果を束ねるだけの薄いラッパーです。新しい評価ロジックは持ちません。1戦略の実行失敗は他戦略の実行を止めません。

戻り値は`strategy_compare_schema_version` / `run_id` / `status`（`"SUCCESS"` / `"PARTIAL_SUCCESS"` / `"FAILED"`。Runnerと同じ3値） / `warnings` / `errors` / `code` / `period` / `strategies`（`{戦略名: RunnerResult}`）というトップレベル構造を持ち、Runner/Pipeline/Benchmark/Summary/Contextと同じ「`run_id`/`status`/`warnings`/`errors`をトップレベルに持つ」という構造上の一貫性に揃えています。`run_id`は未指定時UUID4が自動生成され、各戦略のRunner呼び出しへも同じ値が伝播します。`status`は「実行できた戦略が1つも無ければ`FAILED`、一部の戦略呼び出しが例外で失敗すれば`PARTIAL_SUCCESS`、すべての戦略が`status: SUCCESS`であれば`SUCCESS`、それ以外は`PARTIAL_SUCCESS`」という、既存の値の集約のみで決定されます（新しい判定基準は導入していません）。

### ランキング（`backtest/walkforward_ranking.py`）
Summaryが既に算出済みの`health_check.score`・`stability_score.score`・`benchmark.best_transition.improvement_score`のみを使い、単純な降順ソートを行うだけです。新しい重み付け・総合スコアは導入していません。

### Window別可視化（`debug_ui.py`）
`summary.window_metrics`（既存データ）をそのまま折れ線グラフ（推移）・棒グラフ（Decision件数・Decisionラベル分布）として描画します。新しい計算は行いません。

### CSVエクスポート（`backtest/walkforward_export.py`）
`window_metrics`・`decision_distribution`・Summaryメタ情報・`benchmark_summary`を、それぞれJSON→`pandas.DataFrame`→CSV文字列へ変換するだけの機能です。新しい集計は行いません。

いずれの機能も、Walk Forward一式のコア原則（既存モジュールへの完全委譲・新しい計算ロジックを持たない）を踏襲しています。

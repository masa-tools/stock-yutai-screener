...
【Evaluation Lab: 分析・検証ハブとしての役割】
  ...
  将来の本番画面・PDF/HTMLレポート生成・CSV出力・APIレスポンス等、
  異なる出力先からも同じ関数をそのまま再利用できる。

【render_evaluation_lab() の正式仕様について（重要）】
  Evaluation Labの正式なエントリポイントとして render_evaluation_lab()
  のシグネチャを確定させている。
    - すべての引数はOptionalであり、Noneを渡しても関数内部で
      安全なデフォルト（空dict/空list）へ正規化されるため、
      呼び出し側でNoneチェックは不要。
    - 戻り値のトップレベルキー（rating/confidence/decision/
      decision_report/benchmark/history_summary/
      evaluation_schema_version）は常に存在する
      （値がNoneや空リストになることはあっても、キー自体が
      条件付きで消えることはない）。これにより、呼び出し側は
      "in"チェックやtry/exceptなしにキーへアクセスできる。
    - evaluation_schema_version をトップレベルに持たせることで、
      将来この戻り値の構造を変更する際に、消費側（本番画面・
      レポート生成・API等）がバージョンを見て互換性を判断できる。
    - 将来ファンダメンタル評価・配当評価・AIコメント等を追加する
      場合の受け皿として context 引数（dict）を用意しているが、
      現時点では解釈・加工を一切行わず、指定されればそのまま
      戻り値の "context" キーへ格納するだけに留めている
      （過度な抽象化を避け、現状の読みやすさを優先するため）。
...
#: render_evaluation_lab() の戻り値スキーマのバージョン。
#: 戻り値の構造（トップレベルキー構成）を変更する場合はこの値を
#: 更新し、消費側（本番画面・レポート生成・API等）が互換性を
#: 判断できるようにする。
...
def render_decision_report_view(decision_report: dict[str, Any]) -> dict[str, Any]:
    ...
    Args:
        decision_report: decision_report.build_decision_report() の戻り値。
            空dict {} を渡した場合も例外にはならず、
            report_info={}・decisions={}のdictを返す。
...
def render_benchmark_view(benchmark_result: Optional[dict[str, Any]]) -> Optional[dict[str, Any]]:
    ...
    Args:
        benchmark_result: benchmark.build_benchmark() の戻り値。
            比較対象が無い場合はNoneを渡してよい。

    Returns:
        benchmark_resultがNone、または空dictの場合はNone
        （＝「Benchmark未評価」を表す）。
        それ以外は {"improvement_score", "overall", "summary"} を持つdict。
    """
...
def render_history_summary_view(
    ...
    benchmarks_by_run_id: Optional[dict[str, dict[str, Any]]] = None,
) -> dict[str, Any]:
    ...
    行わない（benchmarks_by_run_id引数で既に計算済みの結果を
    受け取るのみ）。

    Args:
        history_entries: report_history.build_history_entry() の
            戻り値のリスト。Noneまたは空リストの場合、
            rows=[] を返す（呼び出し側でのNoneチェックは不要）。
        benchmarks_by_run_id: {run_id: benchmark.build_benchmark()の戻り値}
            のマッピング（任意）。対応するrun_idが無いエントリの
            improvement_scoreはNoneになる。Noneの場合は空dictとして扱う。
    ...
    Returns:
        ...
        historyが無い場合は {"rows": []} を返す。
    """
    entries = history_entries or []
    benchmarks_by_run_id = benchmarks_by_run_id or {}
    rows: list[dict[str, Any]] = []

    for entry in entries:
        run_id = entry.get("run_id")
        benchmark = benchmarks_by_run_id.get(run_id)
        rows.append({
            "run_id": run_id,
...
def render_evaluation_lab(
    ...
    Evaluation Labの正式なエントリポイント。①Rating → ②Confidence →
    ③Decision → ④Decision Report → ⑤Benchmark → ⑥History Summary の順で、
    対応するrender_*関数を呼び出すだけの薄い統括関数。内部で計算・
    Decision Engine・Benchmark・Report・History Entryの生成は一切
    行わない（それらの戻り値を引数として受け取るだけ）。

    すべての引数はOptionalであり、Noneを渡しても関数内部で安全な
    デフォルト（空dict/空list）へ正規化されるため、呼び出し側で
    事前のNoneチェックは不要。

    Streamlit等のUIライブラリに依存しないため、debug_ui.py・本番画面・
    PDFレポート生成・CSV出力・APIレスポンスなど複数の出力先から
    同じ関数をそのまま呼び出せる。

    Args:
        rating_result: rating.build_rating() 系関数（build_rating() /
            build_rating_from_score_result()）の戻り値。dict型。
            Noneの場合は空dictとして扱われ、"rating"セクションの
            各項目はNoneになる。
        confidence_result: confidence.build_confidence() の戻り値。
            dict型。Noneの場合は空dictとして扱われ、"confidence"
            セクションの各項目はNoneになる。
        decision_result: decision.build_decision() の戻り値。dict型。
            Noneの場合は空dictとして扱われ、"decision"セクションの
            各項目はNoneになる。
        decision_report: decision_report.build_decision_report() の
            戻り値。dict型（"report_info"キー＋Decisionラベルごとの
            キーを持つ）。Noneの場合は空dictとして扱われ、
            "decision_report"セクションは report_info={}・
            decisions={} になる。
        benchmark_result: benchmark.build_benchmark() の戻り値。dict型。
            比較対象となる実験結果が無い場合はNoneを渡す
            （デフォルトNone）。その場合、戻り値の"benchmark"キーの
            値はNoneになる（＝「未評価」を表す）。
        history_entries: report_history.build_history_entry() の
            戻り値のリスト（list[dict]）。実行履歴が無い場合はNone
            または空リストを渡す（デフォルトNone）。その場合、
            "history_summary"の"rows"は空リストになる。
        benchmarks_by_run_id: {run_id: benchmark.build_benchmark()の
            戻り値} のマッピング（dict[str, dict]）。History Summaryの
            各行にImprovement Scoreを付与するために使う（任意）。
            Noneの場合は空dictとして扱われ、全行のimprovement_scoreは
            Noneになる。
        context: 将来のファンダメンタル評価・配当評価・AIコメント等の
            追加シグナルを見据えた予約引数（dict）。現時点では内容の
            解釈・加工を一切行わず、指定された場合のみ戻り値の
            "context"キーへそのまま格納する（デフォルトNone、
            その場合戻り値に"context"キーは含まれない）。

    Returns:
        以下のトップレベルキーを持つJSON互換dict（"context"を除き
        常にすべてのキーが存在する）::

            {
                ...
                "benchmark": render_benchmark_view()の戻り値 | None,
                "history_summary": render_history_summary_view()の戻り値,
                "context": context引数の値,  # contextが指定された場合のみ
            }
    """
    lab: dict[str, Any] = {
        "evaluation_schema_version": EVALUATION_SCHEMA_VERSION,
        "rating": render_rating_view(rating_result or {}),
        "confidence": render_confidence_view(confidence_result or {}),
        "decision": render_decision_view(decision_result or {}),
        "decision_report": render_decision_report_view(decision_report or {}),
        "benchmark": render_benchmark_view(benchmark_result),
        "history_summary": render_history_summary_view(history_entries, benchmarks_by_run_id),
    }

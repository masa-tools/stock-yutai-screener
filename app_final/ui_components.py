...
【v3修正内容】
  Fix①  render_metrics()の配当利回り表示も二重変換バグを修正
  Fix②  render_header()のdisclaimer重複を整理
         (app.pyでも同内容を描画していたため1か所に統一)
"""
...
.main .block-container { padding-top: 1.5rem; padding-bottom: 3rem; max-width: 1050px; }

.app-header {
    background: linear-gradient(135deg, #fce4ec, #f8bbd0, #e1bee7);
    border-radius: 20px; padding: 1.6rem 2rem; text-align: center;
    margin-bottom: 1.2rem; box-shadow: 0 4px 18px rgba(233,30,99,0.08);
}
.app-header h1 {
    font-family: 'Zen Maru Gothic', sans-serif;
    font-size: 1.9rem; font-weight: 700; color: #880e4f; margin: 0;
}
.app-header p { color: #ad1457; font-size: 0.9rem; margin-top: 0.3rem; }

.stTabs [data-baseweb="tab-list"] {
    gap: 6px; border-bottom: 2px solid #f8bbd0; background: transparent;
}
.stTabs [data-baseweb="tab"] {
    background: #fff0f5; border-radius: 12px 12px 0 0 !important;
    padding: 0.5rem 1.3rem; font-weight: 600; color: #c2185b;
    border: 1px solid #f8bbd0; border-bottom: none;
}
.stTabs [aria-selected="true"] { background: #fce4ec !important; color: #880e4f !important; }

.card {
    background: #fff; border-radius: 18px;
    padding: 1.3rem 1.5rem; box-shadow: 0 2px 14px rgba(200,100,120,0.08);
    border: 1px solid #fce4ec; margin-bottom: 0.9rem;
}
.m-card {
    background: linear-gradient(135deg, #fff9fb, #fce4ec);
    border-radius: 16px; padding: 1rem 1.1rem;
    border: 1px solid #f8bbd0; text-align: center; height: 100%;
    box-shadow: 0 2px 10px rgba(200,100,120,0.06);
}
.m-label  { font-size: 0.7rem; color: #ad1457; font-weight: 600; letter-spacing: 0.04em; text-transform: uppercase; margin-bottom: 0.2rem; }
.m-value  { font-family: 'Zen Maru Gothic', sans-serif; font-size: 1.45rem; font-weight: 700; color: #3d2b1f; }
...
.sec-title {
    ...
    margin: 1.3rem 0 0.6rem; padding-left: 0.7rem;
    border-left: 4px solid #f48fb1;
}
.score-badge {
    display: inline-block;
    background: linear-gradient(135deg, #f48fb1, #ce93d8);
    color: #fff; border-radius: 50px; padding: 0.3rem 1rem;
    font-weight: 700; font-size: 1rem; box-shadow: 0 2px 8px rgba(206,147,216,0.4);
}
...
.bar-wrap { background: #fce4ec; border-radius: 50px; height: 10px; overflow: hidden; margin-top: 0.25rem; }
.bar-fill { height: 100%; border-radius: 50px; background: linear-gradient(90deg, #f48fb1, #ce93d8); }
.disclaimer {
    background: #fff9c4; border-left: 4px solid #ffd54f;
    border-radius: 0 10px 10px 0; padding: 0.6rem 0.9rem;
    font-size: 0.78rem; color: #5d4037; margin: 0.7rem 0;
}
.stButton button {
    ...
    padding: 0.5rem 1.7rem !important; font-weight: 700 !important;
    box-shadow: 0 3px 12px rgba(244,143,177,0.4) !important;
}
.stButton button:hover { transform: translateY(-2px) !important; }
.stTextInput input {
    border-radius: 12px !important; border: 2px solid #f8bbd0 !important;
...
.footer { text-align: center; font-size: 0.73rem; color: #bbb;
          margin-top: 2.5rem; padding-top: 1rem; border-top: 1px solid #fce4ec; }
</style>
...
# ════════════════════════════════
def render_header():
    """アプリのタイトルを表示する（disclaimerはapp.pyで1回だけ表示）"""
    st.markdown("""
<div class="app-header">
    <h1>株主優待スクリーナー</h1>
    <p>財務・テクニカル・配当・株主優待の4軸で日本株をやさしく分析します</p>
</div>
""", unsafe_allow_html=True)
...
def _safe_dividend_display(dy) -> tuple[str, str]:
    """
    配当利回りを表示用文字列とヒント文字列に変換する。
    recommend.pyと同じ安全な変換ロジックを使用。
    """
    if dy is None:
        return "無配当", ""
    try:
        ...
        if pct < 0.1 or pct > 30:
            return "―", ""
        div_str  = f"{pct:.2f}%"
        div_hint = ("理想的な高配当 ✨" if 3 <= pct <= 5.5
                    else "安定した配当" if pct >= 1.5
                    else "低配当（成長型）")
        return div_str, div_hint

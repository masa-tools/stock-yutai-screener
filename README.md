...
├── dividend_sim.py          ← 💰 配当シミュレーター（+ボタン修正済み）
...
```bash
git add -A
git commit -m "v6.0: 288銘柄・不具合修正・詳細展開"
...
|---|---------|
| ① | 候補銘柄 125→**288銘柄**に拡大 |
| ② | カレンダー 288銘柄対応・日本語名・12月期企業対応 |
| ③ | ウォッチリスト session_stateキー分離で確実動作 |
| ④ | シミュレーター number_input の key廃止で+ボタン修正 |
| ⑤ | おすすめ詳細ボタン→その場で展開表示 |
| ⑥ | 根拠に「負債比率・増配余地・移動平均」等を追加 |
| ⑦ | @st.cache_data でyfinanceアクセス削減 |
| ⑧ | ボタンkey全てユニーク化（重複解消）|
